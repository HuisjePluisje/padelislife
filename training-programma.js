(function () {
  ...
  function getBlockCopy(block) {
    if (block.exerciseId) {
      return `
        <strong>${escapeHtml(getBlockLabel(block))} · ${escapeHtml(block.exerciseTitle)}</strong>
        ...
        return `
          <article class="program-exercise-card">
            <div class="program-exercise-card-head">
              <span class="module-format">Blok ${block.sequenceIndex + 1}</span>
              <span class="meta-pill">${escapeHtml(String(block.minutes))} minuten</span>
            </div>
  ...
  function renderBlockTimeline() {
    return program.blocks
      .map(
        (block) => `
          <li class="program-block-item${block.transfer ? " program-block-transfer" : ""}${block.log ? " program-block-log" : ""}">
