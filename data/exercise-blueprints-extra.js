window.EXTRA_EXERCISE_BLUEPRINTS = [
  {
    "id": "13",
    "category": "TRANSITIE & INITIATIEF",
    "title": "Backglass Forehand Reset → Chiquita Entry",
    "shortTitle": "Backglass Forehand Reset",
    "summary": "Resetten, kort maken en daarna het net terugpakken.",
    "context": "Je wordt diep vastgezet op je forehandhoek en moet eerst tijd kopen, daarna zelf weer de bal laag naar voren brengen.",
    "matchContext": "Spelers overleven de eerste bal, maar blijven vervolgens in verdediging hangen.",
    "level": "beginner",
    "levelLabel": "Beginner",
    "knltb": "8-9",
    "type": "defense",
    "typeLabel": "Defense & recovery",
    "sequence": "3-ball sequence",
    "repeats": "20 herhalingen",
    "totalBalls": "60 ballen totaal",
    "categoryColor": "#0D5517",
    "playerVariant": {
      "left": [
        "Voor linkshandig rechts krijgt backglass forehand reset meer nadruk op forehand-reddingen, uit de hoek draaien en via cross of midden tijd kopen.",
        "De herstelpas richt zich vaker op het opnieuw vrijmaken van de forehandhelft en het terugclaimen van middenruimte.",
        "Laat de speler herkennen wanneer een hoge reset slimmer is dan een vlakke uitweg."
      ],
      "right": [
        "Voor rechtshandig links ligt backglass forehand reset meer op backhand-organisatie, compacter glasgebruik en rust houden richting lijnzijde.",
        "De speler moet sneller kiezen tussen blokkeren, hoog resetten of via midden neutraliseren.",
        "Herstel blijft leidend: eerst uit de druk, daarna pas denken aan omschakelen."
      ]
    },
    "goal": [
      {
        "title": "Hoofddoel",
        "text": "Resetten, kort maken en daarna het net terugpakken."
      },
      {
        "type": "list",
        "title": "Tactische doelen",
        "items": [
          "Situatie herkennen: trigger de eerste bal dwingt je uit comfort. Actie: lees vroeg en kies een functionele eerste oplossing. Waarom: zonder heldere eerste keuze ontstaat chaos.",
          "Patroon afmaken: trigger je eerste bal heeft tijd of ruimte gekocht. Actie: gebruik de tweede bal om richting te geven aan de rally. Waarom: hier verschuift initiatief of controle.",
          "Positie vastleggen: trigger na de laatste bal van de cyclus. Actie: herstel naar de plek waar de volgende bal logisch wordt. Waarom: goede training eindigt niet bij contact, maar bij bruikbare positie."
        ]
      }
    ],
    "setup": [
      {
        "title": "Positie",
        "text": "Start in forehandhoek, 0,5-1 meter voor achterglas."
      },
      {
        "title": "Setup",
        "text": "Lage basis, racket vóór je romp, schouders neutraal."
      },
      {
        "title": "Setup",
        "text": "Voor linkshandig rechts: eerste stap uit de hoek met buitenbeen, daarna direct terug richting T."
      }
    ],
    "targets": [
      {
        "name": "RESET CROSS",
        "text": "Hoge diepe lob naar cross om tijd te winnen."
      },
      {
        "name": "CHIQUITA-ZONE",
        "text": "Korte lage bal onder servicelijn of naar voeten."
      },
      {
        "name": "NET-HOMEBASE",
        "text": "Na bal 2 direct weer voorin landen."
      }
    ],
    "steps": [
      {
        "title": "Bal 1",
        "text": "Gebruik glas, speel hoge reset-lob cross. Waarom: Tijd kopen en uit de hoek komen. Doelzone: RESET CROSS"
      },
      {
        "title": "Bal 2",
        "text": "Neem vroeg en speel chiquita laag naar korte zone. Waarom: Ruimte winnen zonder risico. Doelzone: CHIQUITA-ZONE"
      },
      {
        "title": "Bal 3",
        "text": "Stap door en speel controle- of punch-volley. Waarom: Aanval overnemen en vasthouden. Doelzone: NET-HOMEBASE"
      }
    ],
    "score": [
      "Bal 1 correct uitgevoerd: __/20",
      "Bal 2 correct uitgevoerd: __/20",
      "Bal 3 correct uitgevoerd: __/20",
      "Fouten totaal: __",
      "Extra KPI: juiste positie na laatste bal: __/20"
    ],
    "coaching": [
      "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
      "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
      "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
    ],
    "machine": [
      "Bal 1: Diepe backglass-forehand, medium-hard. → Gebruik glas, speel hoge reset-lob cross. (Tijd kopen en uit de hoek komen.)",
      "Bal 2: Halflange middenbal, medium. → Neem vroeg en speel chiquita laag naar korte zone. (Ruimte winnen zonder risico.)",
      "Bal 3: Lage netbal, medium-slow. → Stap door en speel controle- of punch-volley. (Aanval overnemen en vasthouden.)"
    ],
    "coachView": [
      {
        "title": "Wat gebeurt er nu vaak?",
        "bullets": [
          "Spelers overleven de eerste bal, maar blijven vervolgens in verdediging hangen.",
          "De speler ziet de eerste moeilijke bal wel, maar denkt nog te weinig in een volledig patroon.",
          "Het herstel of de vervolgkeuze komt te laat."
        ]
      },
      {
        "title": "Waarom werkt dit niet?",
        "bullets": [
          "De slag en de positie erna worden nog los van elkaar gezien.",
          "Er is te weinig onderscheid tussen neutraliseren, opbouwen en drukken.",
          "Zonder duidelijk patroon blijft de speler reactief."
        ]
      },
      {
        "title": "Wat je gaat leren",
        "bullets": [
          "Resetten, kort maken en daarna het net terugpakken.",
          "Niet alleen de bal raken, maar ook de volgende positie voorbereiden.",
          "Een herhaalbaar patroon bouwen dat ook onder druk bruikbaar blijft."
        ]
      },
      {
        "title": "Beslisregel",
        "bullets": [
          "🟢 Groen: druk mag omhoog; kies voeten of duidelijke open ruimte als je vóór de bal bent.",
          "🟠 Oranje: kies body of T; controle blijft belangrijker dan spektakel.",
          "🔴 Rood: koop tijd, speel hoog of neutraal en herstel eerst."
        ]
      },
      {
        "title": "Coach Tips",
        "bullets": [
          "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
          "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
          "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
        ]
      }
    ]
  },
  {
    "id": "14",
    "category": "NET BEHOUDEN",
    "title": "Lob Read → Bandeja Body → Net Hold",
    "shortTitle": "Lob Read",
    "summary": "Lob vroeg lezen, body-target kiezen en meteen weer naar voren bewegen.",
    "context": "Je staat aan het net, leest de lob en moet de bandeja zo spelen dat je het net niet weggeeft.",
    "matchContext": "De overhead is vaak technisch oké, maar tactisch te hard, te mooi of te vrijblijvend.",
    "level": "average",
    "levelLabel": "Gemiddeld",
    "knltb": "6-7",
    "type": "net",
    "typeLabel": "Netdominantie",
    "sequence": "4-ball sequence",
    "repeats": "20 herhalingen",
    "totalBalls": "80 ballen totaal",
    "categoryColor": "#0D3B66",
    "playerVariant": {
      "left": [
        "Voor linkshandig rechts ligt de nadruk op forehand-overheads, bodydruk en het afsluiten van middenballen vanuit lob read.",
        "Het herstel na overhead of eerste volley blijft iets meer gericht op het bewaken van het midden en de forehandzijde.",
        "Laat de speler na drukballen sneller herkennen wanneer de volgende bal naar voeten/body gaat in plaats van naar de kleine hoek."
      ],
      "right": [
        "Voor rechtshandig links verschuift lob read meer naar backhand-ordening, lijnbewaking en compacter voetenwerk in de volleyzone.",
        "De nadruk ligt iets vaker op het sluiten van de lijn en daarna pas op het meenemen van middenruimte.",
        "Werk met dezelfde sequentie, maar laat de speler andere contactpunten en herstelhoeken voelen."
      ]
    },
    "goal": [
      {
        "title": "Hoofddoel",
        "text": "Lob vroeg lezen, body-target kiezen en meteen weer naar voren bewegen."
      },
      {
        "type": "list",
        "title": "Tactische doelen",
        "items": [
          "Situatie herkennen: trigger de eerste bal dwingt je uit comfort. Actie: lees vroeg en kies een functionele eerste oplossing. Waarom: zonder heldere eerste keuze ontstaat chaos.",
          "Patroon afmaken: trigger je eerste bal heeft tijd of ruimte gekocht. Actie: gebruik de tweede bal om richting te geven aan de rally. Waarom: hier verschuift initiatief of controle.",
          "Positie vastleggen: trigger na de laatste bal van de cyclus. Actie: herstel naar de plek waar de volgende bal logisch wordt. Waarom: goede training eindigt niet bij contact, maar bij bruikbare positie."
        ]
      }
    ],
    "setup": [
      {
        "title": "Positie",
        "text": "Start vanuit net-homebase, racket hoog."
      },
      {
        "title": "Setup",
        "text": "Voeten licht actief, klaar voor crossover naar achter."
      },
      {
        "title": "Setup",
        "text": "Voor linkshandig rechts: draai open naar forehand-overhead en herstel meteen voorwaarts."
      }
    ],
    "targets": [
      {
        "name": "BANDEJA BODY",
        "text": "Diep naar lichaam of midden."
      },
      {
        "name": "FORWARD RECOVERY",
        "text": "Twee snelle passen terug naar voren."
      },
      {
        "name": "NET LOCK",
        "text": "Laatste volley laag naar T of voeten."
      }
    ],
    "steps": [
      {
        "title": "Bal 1",
        "text": "Open rustig naar T. Waarom: Ritme en basis neerzetten. Doelzone: T"
      },
      {
        "title": "Bal 2",
        "text": "Iets meer druk, nog steeds compact. Waarom: Voorbereiden op overheadfase. Doelzone: BODY/T"
      },
      {
        "title": "Bal 3",
        "text": "Speel gecontroleerde bandeja naar body. Waarom: Netvoordeel vasthouden. Doelzone: BANDEJA BODY"
      },
      {
        "title": "Bal 4",
        "text": "Pak de eerste volley opnieuw laag. Waarom: Net vastzetten. Doelzone: NET LOCK"
      }
    ],
    "score": [
      "Bal 1 correct uitgevoerd: __/20",
      "Bal 2 correct uitgevoerd: __/20",
      "Bal 3 correct uitgevoerd: __/20",
      "Bal 4 correct uitgevoerd: __/20",
      "Fouten totaal: __",
      "Extra KPI: juiste positie na laatste bal: __/20"
    ],
    "coaching": [
      "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
      "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
      "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
    ],
    "machine": [
      "Bal 1: Controlevolley, medium. → Open rustig naar T. (Ritme en basis neerzetten.)",
      "Bal 2: Tweede volley, medium. → Iets meer druk, nog steeds compact. (Voorbereiden op overheadfase.)",
      "Bal 3: Hoge lob, medium-slow. → Speel gecontroleerde bandeja naar body. (Netvoordeel vasthouden.)",
      "Bal 4: Netbal na recovery, medium. → Pak de eerste volley opnieuw laag. (Net vastzetten.)"
    ],
    "coachView": [
      {
        "title": "Wat gebeurt er nu vaak?",
        "bullets": [
          "De overhead is vaak technisch oké, maar tactisch te hard, te mooi of te vrijblijvend.",
          "De speler ziet de eerste moeilijke bal wel, maar denkt nog te weinig in een volledig patroon.",
          "Het herstel of de vervolgkeuze komt te laat."
        ]
      },
      {
        "title": "Waarom werkt dit niet?",
        "bullets": [
          "De slag en de positie erna worden nog los van elkaar gezien.",
          "Er is te weinig onderscheid tussen neutraliseren, opbouwen en drukken.",
          "Zonder duidelijk patroon blijft de speler reactief."
        ]
      },
      {
        "title": "Wat je gaat leren",
        "bullets": [
          "Lob vroeg lezen, body-target kiezen en meteen weer naar voren bewegen.",
          "Niet alleen de bal raken, maar ook de volgende positie voorbereiden.",
          "Een herhaalbaar patroon bouwen dat ook onder druk bruikbaar blijft."
        ]
      },
      {
        "title": "Beslisregel",
        "bullets": [
          "🟢 Groen: druk mag omhoog; kies voeten of duidelijke open ruimte als je vóór de bal bent.",
          "🟠 Oranje: kies body of T; controle blijft belangrijker dan spektakel.",
          "🔴 Rood: koop tijd, speel hoog of neutraal en herstel eerst."
        ]
      },
      {
        "title": "Coach Tips",
        "bullets": [
          "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
          "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
          "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
        ]
      }
    ]
  },
  {
    "id": "15",
    "category": "RETURN & SLUITEN",
    "title": "Return Body Block → Deep Middle → Close Middle",
    "shortTitle": "Return Body Block",
    "summary": "Compact returneren, diep midden vinden en daarna bewust het midden sluiten.",
    "context": "Na service-return wil je niet alleen de bal terugbrengen, maar meteen het midden klein maken.",
    "matchContext": "Veel returns komen wel terug, maar geven de server te veel ruimte voor bal drie.",
    "level": "average",
    "levelLabel": "Gemiddeld",
    "knltb": "6-7",
    "type": "return",
    "typeLabel": "Return & opening",
    "sequence": "3-ball sequence",
    "repeats": "20 herhalingen",
    "totalBalls": "60 ballen totaal",
    "categoryColor": "#4A1C6A",
    "playerVariant": {
      "left": [
        "Voor linkshandig rechts draait return body block vaker om een forehand-georiënteerde vervolgkeuze na return of service-plus-één.",
        "De speler bewaakt het midden actiever en kiest eerder voor body of midden dan voor de smalle lijn.",
        "Doorstappen gebeurt alleen wanneer de return of eerste bal ook echt ruimte wint."
      ],
      "right": [
        "Voor rechtshandig links verschuift return body block meer naar backhandcontrole, cross-openingen en later instappen op de volgende bal.",
        "Laat de speler de return iets meer koppelen aan lijnsluiting en partnerlogica.",
        "Werk met dezelfde oefenopbouw, maar andere besluitmomenten rond midden en lijn."
      ]
    },
    "goal": [
      {
        "title": "Hoofddoel",
        "text": "Compact returneren, diep midden vinden en daarna bewust het midden sluiten."
      },
      {
        "type": "list",
        "title": "Tactische doelen",
        "items": [
          "Situatie herkennen: trigger de eerste bal dwingt je uit comfort. Actie: lees vroeg en kies een functionele eerste oplossing. Waarom: zonder heldere eerste keuze ontstaat chaos.",
          "Patroon afmaken: trigger je eerste bal heeft tijd of ruimte gekocht. Actie: gebruik de tweede bal om richting te geven aan de rally. Waarom: hier verschuift initiatief of controle.",
          "Positie vastleggen: trigger na de laatste bal van de cyclus. Actie: herstel naar de plek waar de volgende bal logisch wordt. Waarom: goede training eindigt niet bij contact, maar bij bruikbare positie."
        ]
      }
    ],
    "setup": [
      {
        "title": "Positie",
        "text": "Start iets achter baseline in returnhouding."
      },
      {
        "title": "Setup",
        "text": "Racket compact, contactpunt vóór de heup."
      },
      {
        "title": "Setup",
        "text": "Voor linkshandig rechts: backhand-return richting midden/body als standaard patroon."
      }
    ],
    "targets": [
      {
        "name": "BODY BLOCK",
        "text": "Compacte return op lichaam of midden."
      },
      {
        "name": "DEEP MIDDLE",
        "text": "Tweede bal diep door midden."
      },
      {
        "name": "CLOSE MIDDLE",
        "text": "Na contact meteen midden beschermen."
      }
    ],
    "steps": [
      {
        "title": "Bal 1",
        "text": "Blok de return compact terug naar body/midden. Waarom: Server uit ritme halen. Doelzone: BODY BLOCK"
      },
      {
        "title": "Bal 2",
        "text": "Speel diep door het midden. Waarom: Ruimte klein houden. Doelzone: DEEP MIDDLE"
      },
      {
        "title": "Bal 3",
        "text": "Herstel en sluit het midden met je volgende bal of positionering. Waarom: Tegenaanval afremmen. Doelzone: CLOSE MIDDLE"
      }
    ],
    "score": [
      "Bal 1 correct uitgevoerd: __/20",
      "Bal 2 correct uitgevoerd: __/20",
      "Bal 3 correct uitgevoerd: __/20",
      "Fouten totaal: __",
      "Extra KPI: juiste positie na laatste bal: __/20"
    ],
    "coaching": [
      "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
      "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
      "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
    ],
    "machine": [
      "Bal 1: Service/returnbal op lichaam, medium-fast. → Blok de return compact terug naar body/midden. (Server uit ritme halen.)",
      "Bal 2: Oplopende rallybal, medium. → Speel diep door het midden. (Ruimte klein houden.)",
      "Bal 3: Vervolgvolley of drukbal, medium. → Herstel en sluit het midden met je volgende bal of positionering. (Tegenaanval afremmen.)"
    ],
    "coachView": [
      {
        "title": "Wat gebeurt er nu vaak?",
        "bullets": [
          "Veel returns komen wel terug, maar geven de server te veel ruimte voor bal drie.",
          "De speler ziet de eerste moeilijke bal wel, maar denkt nog te weinig in een volledig patroon.",
          "Het herstel of de vervolgkeuze komt te laat."
        ]
      },
      {
        "title": "Waarom werkt dit niet?",
        "bullets": [
          "De slag en de positie erna worden nog los van elkaar gezien.",
          "Er is te weinig onderscheid tussen neutraliseren, opbouwen en drukken.",
          "Zonder duidelijk patroon blijft de speler reactief."
        ]
      },
      {
        "title": "Wat je gaat leren",
        "bullets": [
          "Compact returneren, diep midden vinden en daarna bewust het midden sluiten.",
          "Niet alleen de bal raken, maar ook de volgende positie voorbereiden.",
          "Een herhaalbaar patroon bouwen dat ook onder druk bruikbaar blijft."
        ]
      },
      {
        "title": "Beslisregel",
        "bullets": [
          "🟢 Groen: druk mag omhoog; kies voeten of duidelijke open ruimte als je vóór de bal bent.",
          "🟠 Oranje: kies body of T; controle blijft belangrijker dan spektakel.",
          "🔴 Rood: koop tijd, speel hoog of neutraal en herstel eerst."
        ]
      },
      {
        "title": "Coach Tips",
        "bullets": [
          "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
          "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
          "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
        ]
      }
    ]
  },
  {
    "id": "16",
    "category": "DEFENSE & HERSTEL",
    "title": "Corner Defense Left → High Reset → Rebuild Line",
    "shortTitle": "Corner Defense Left",
    "summary": "Hoge reset als eerste keuze, daarna gecontroleerd opnieuw richting lijn of T bouwen.",
    "context": "Je wordt linksachter in de hoek geduwd en moet de rally eerst vertragen, daarna weer opbouwen.",
    "matchContext": "Spelers willen uit de hoek te snel weer aanvallen en verliezen daardoor de rally op bal twee.",
    "level": "average",
    "levelLabel": "Gemiddeld",
    "knltb": "6-7",
    "type": "defense",
    "typeLabel": "Defense & recovery",
    "sequence": "3-ball sequence",
    "repeats": "20 herhalingen",
    "totalBalls": "60 ballen totaal",
    "categoryColor": "#6B4226",
    "playerVariant": {
      "left": [
        "Voor linkshandig rechts krijgt corner defense left meer nadruk op forehand-reddingen, uit de hoek draaien en via cross of midden tijd kopen.",
        "De herstelpas richt zich vaker op het opnieuw vrijmaken van de forehandhelft en het terugclaimen van middenruimte.",
        "Laat de speler herkennen wanneer een hoge reset slimmer is dan een vlakke uitweg."
      ],
      "right": [
        "Voor rechtshandig links ligt corner defense left meer op backhand-organisatie, compacter glasgebruik en rust houden richting lijnzijde.",
        "De speler moet sneller kiezen tussen blokkeren, hoog resetten of via midden neutraliseren.",
        "Herstel blijft leidend: eerst uit de druk, daarna pas denken aan omschakelen."
      ]
    },
    "goal": [
      {
        "title": "Hoofddoel",
        "text": "Hoge reset als eerste keuze, daarna gecontroleerd opnieuw richting lijn of T bouwen."
      },
      {
        "type": "list",
        "title": "Tactische doelen",
        "items": [
          "Situatie herkennen: trigger de eerste bal dwingt je uit comfort. Actie: lees vroeg en kies een functionele eerste oplossing. Waarom: zonder heldere eerste keuze ontstaat chaos.",
          "Patroon afmaken: trigger je eerste bal heeft tijd of ruimte gekocht. Actie: gebruik de tweede bal om richting te geven aan de rally. Waarom: hier verschuift initiatief of controle.",
          "Positie vastleggen: trigger na de laatste bal van de cyclus. Actie: herstel naar de plek waar de volgende bal logisch wordt. Waarom: goede training eindigt niet bij contact, maar bij bruikbare positie."
        ]
      }
    ],
    "setup": [
      {
        "title": "Positie",
        "text": "Start in linkerhoek achterin."
      },
      {
        "title": "Setup",
        "text": "Lage basis, lichaam neutraal, klaar om glas te lezen."
      },
      {
        "title": "Setup",
        "text": "Voor linkshandig rechts: extra aandacht voor backhandhoek en eerste herstelpas naar midden."
      }
    ],
    "targets": [
      {
        "name": "HIGH RESET",
        "text": "Hoge veilige lob of lift."
      },
      {
        "name": "REBUILD ZONE",
        "text": "Tweede bal veilig naar T of lijn."
      },
      {
        "name": "HOMEBASE",
        "text": "Na elke slag terug naar werkbare verdedigingspositie."
      }
    ],
    "steps": [
      {
        "title": "Bal 1",
        "text": "Speel hoog en veilig uit de hoek. Waarom: Chaos eerst neutraliseren. Doelzone: HIGH RESET"
      },
      {
        "title": "Bal 2",
        "text": "Bouw opnieuw op richting T of gecontroleerde lijn. Waarom: Van chaos naar controle. Doelzone: REBUILD ZONE"
      },
      {
        "title": "Bal 3",
        "text": "Blijf in patroon en herstel opnieuw. Waarom: Rally verlengen met kwaliteit. Doelzone: HOMEBASE"
      }
    ],
    "score": [
      "Bal 1 correct uitgevoerd: __/20",
      "Bal 2 correct uitgevoerd: __/20",
      "Bal 3 correct uitgevoerd: __/20",
      "Fouten totaal: __",
      "Extra KPI: juiste positie na laatste bal: __/20"
    ],
    "coaching": [
      "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
      "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
      "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
    ],
    "machine": [
      "Bal 1: Diepe hoekbal, medium-hard. → Speel hoog en veilig uit de hoek. (Chaos eerst neutraliseren.)",
      "Bal 2: Midden- of zijlijnbal, medium. → Bouw opnieuw op richting T of gecontroleerde lijn. (Van chaos naar controle.)",
      "Bal 3: Neutrale vervolgbal, medium. → Blijf in patroon en herstel opnieuw. (Rally verlengen met kwaliteit.)"
    ],
    "coachView": [
      {
        "title": "Wat gebeurt er nu vaak?",
        "bullets": [
          "Spelers willen uit de hoek te snel weer aanvallen en verliezen daardoor de rally op bal twee.",
          "De speler ziet de eerste moeilijke bal wel, maar denkt nog te weinig in een volledig patroon.",
          "Het herstel of de vervolgkeuze komt te laat."
        ]
      },
      {
        "title": "Waarom werkt dit niet?",
        "bullets": [
          "De slag en de positie erna worden nog los van elkaar gezien.",
          "Er is te weinig onderscheid tussen neutraliseren, opbouwen en drukken.",
          "Zonder duidelijk patroon blijft de speler reactief."
        ]
      },
      {
        "title": "Wat je gaat leren",
        "bullets": [
          "Hoge reset als eerste keuze, daarna gecontroleerd opnieuw richting lijn of T bouwen.",
          "Niet alleen de bal raken, maar ook de volgende positie voorbereiden.",
          "Een herhaalbaar patroon bouwen dat ook onder druk bruikbaar blijft."
        ]
      },
      {
        "title": "Beslisregel",
        "bullets": [
          "🟢 Groen: druk mag omhoog; kies voeten of duidelijke open ruimte als je vóór de bal bent.",
          "🟠 Oranje: kies body of T; controle blijft belangrijker dan spektakel.",
          "🔴 Rood: koop tijd, speel hoog of neutraal en herstel eerst."
        ]
      },
      {
        "title": "Coach Tips",
        "bullets": [
          "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
          "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
          "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
        ]
      }
    ]
  },
  {
    "id": "17",
    "category": "CHAOS NAAR CONTROLE",
    "title": "Smash Retrieval → Reset Cross → Reclaim T",
    "shortTitle": "Smash Retrieval",
    "summary": "Eerst hoog cross resetten, daarna de T opnieuw innemen en pas dan weer denken aan initiatief.",
    "context": "Na het terughalen van een smash moet je niet blind wegslaan, maar de rally weer ordenen.",
    "matchContext": "Na een spectaculaire retrieval volgt vaak een gehaaste tweede bal.",
    "level": "advanced",
    "levelLabel": "Advanced",
    "knltb": "1-5",
    "type": "net",
    "typeLabel": "Netdominantie",
    "sequence": "3-ball sequence",
    "repeats": "20 herhalingen",
    "totalBalls": "60 ballen totaal",
    "categoryColor": "#0D5517",
    "playerVariant": {
      "left": [
        "Voor linkshandig rechts ligt de nadruk op forehand-overheads, bodydruk en het afsluiten van middenballen vanuit smash retrieval.",
        "Het herstel na overhead of eerste volley blijft iets meer gericht op het bewaken van het midden en de forehandzijde.",
        "Laat de speler na drukballen sneller herkennen wanneer de volgende bal naar voeten/body gaat in plaats van naar de kleine hoek."
      ],
      "right": [
        "Voor rechtshandig links verschuift smash retrieval meer naar backhand-ordening, lijnbewaking en compacter voetenwerk in de volleyzone.",
        "De nadruk ligt iets vaker op het sluiten van de lijn en daarna pas op het meenemen van middenruimte.",
        "Werk met dezelfde sequentie, maar laat de speler andere contactpunten en herstelhoeken voelen."
      ]
    },
    "goal": [
      {
        "title": "Hoofddoel",
        "text": "Eerst hoog cross resetten, daarna de T opnieuw innemen en pas dan weer denken aan initiatief."
      },
      {
        "type": "list",
        "title": "Tactische doelen",
        "items": [
          "Situatie herkennen: trigger de eerste bal dwingt je uit comfort. Actie: lees vroeg en kies een functionele eerste oplossing. Waarom: zonder heldere eerste keuze ontstaat chaos.",
          "Patroon afmaken: trigger je eerste bal heeft tijd of ruimte gekocht. Actie: gebruik de tweede bal om richting te geven aan de rally. Waarom: hier verschuift initiatief of controle.",
          "Positie vastleggen: trigger na de laatste bal van de cyclus. Actie: herstel naar de plek waar de volgende bal logisch wordt. Waarom: goede training eindigt niet bij contact, maar bij bruikbare positie."
        ]
      }
    ],
    "setup": [
      {
        "title": "Positie",
        "text": "Start diep achterin, iets uit midden."
      },
      {
        "title": "Setup",
        "text": "Racket laag maar klaar voor snelle reactiestap."
      },
      {
        "title": "Setup",
        "text": "Voor linkshandig rechts: retrieval vaak naar forehand- of middenzone, daarna snel terug naar T."
      }
    ],
    "targets": [
      {
        "name": "RETRIEVAL",
        "text": "Eerste reddingsbal in spel houden."
      },
      {
        "name": "RESET CROSS",
        "text": "Tweede bal hoog en diep cross."
      },
      {
        "name": "RECLAIM T",
        "text": "Terug naar middencontrole."
      }
    ],
    "steps": [
      {
        "title": "Bal 1",
        "text": "Houd de bal in spel met minimale chaos. Waarom: Punt levend houden. Doelzone: RETRIEVAL"
      },
      {
        "title": "Bal 2",
        "text": "Reset hoog cross. Waarom: Tijd en ruimte terugkopen. Doelzone: RESET CROSS"
      },
      {
        "title": "Bal 3",
        "text": "Neem vroeg en ga terug naar T-logica. Waarom: Rally opnieuw stabiliseren. Doelzone: RECLAIM T"
      }
    ],
    "score": [
      "Bal 1 correct uitgevoerd: __/20",
      "Bal 2 correct uitgevoerd: __/20",
      "Bal 3 correct uitgevoerd: __/20",
      "Fouten totaal: __",
      "Extra KPI: juiste positie na laatste bal: __/20"
    ],
    "coaching": [
      "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
      "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
      "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
    ],
    "machine": [
      "Bal 1: Smash retrieval, snel/lastig. → Houd de bal in spel met minimale chaos. (Punt levend houden.)",
      "Bal 2: Bruikbare volgende bal, medium. → Reset hoog cross. (Tijd en ruimte terugkopen.)",
      "Bal 3: Neutrale middenbal, medium. → Neem vroeg en ga terug naar T-logica. (Rally opnieuw stabiliseren.)"
    ],
    "coachView": [
      {
        "title": "Wat gebeurt er nu vaak?",
        "bullets": [
          "Na een spectaculaire retrieval volgt vaak een gehaaste tweede bal.",
          "De speler ziet de eerste moeilijke bal wel, maar denkt nog te weinig in een volledig patroon.",
          "Het herstel of de vervolgkeuze komt te laat."
        ]
      },
      {
        "title": "Waarom werkt dit niet?",
        "bullets": [
          "De slag en de positie erna worden nog los van elkaar gezien.",
          "Er is te weinig onderscheid tussen neutraliseren, opbouwen en drukken.",
          "Zonder duidelijk patroon blijft de speler reactief."
        ]
      },
      {
        "title": "Wat je gaat leren",
        "bullets": [
          "Eerst hoog cross resetten, daarna de T opnieuw innemen en pas dan weer denken aan initiatief.",
          "Niet alleen de bal raken, maar ook de volgende positie voorbereiden.",
          "Een herhaalbaar patroon bouwen dat ook onder druk bruikbaar blijft."
        ]
      },
      {
        "title": "Beslisregel",
        "bullets": [
          "🟢 Groen: druk mag omhoog; kies voeten of duidelijke open ruimte als je vóór de bal bent.",
          "🟠 Oranje: kies body of T; controle blijft belangrijker dan spektakel.",
          "🔴 Rood: koop tijd, speel hoog of neutraal en herstel eerst."
        ]
      },
      {
        "title": "Coach Tips",
        "bullets": [
          "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
          "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
          "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
        ]
      }
    ]
  },
  {
    "id": "18",
    "category": "SERVICEPATROON",
    "title": "Serve + Ball 3 Plan Right Side",
    "shortTitle": "Serve + Ball 3 Plan Right Side",
    "summary": "Service gebruiken als start van patroon: serveren, bal drie sturen, netpositie vastleggen.",
    "context": "Je serveert vanaf rechts en wilt een duidelijk plan trainen voor service, bal drie en je eerste positie aan het net.",
    "matchContext": "De service wordt te vaak los gezien van de rally die erna komt.",
    "level": "advanced",
    "levelLabel": "Advanced",
    "knltb": "1-5",
    "type": "return",
    "typeLabel": "Return & opening",
    "sequence": "3-ball sequence",
    "repeats": "20 herhalingen",
    "totalBalls": "60 ballen totaal",
    "categoryColor": "#4A1C6A",
    "playerVariant": {
      "left": [
        "Voor linkshandig rechts draait serve + ball 3 plan right side vaker om een forehand-georiënteerde vervolgkeuze na return of service-plus-één.",
        "De speler bewaakt het midden actiever en kiest eerder voor body of midden dan voor de smalle lijn.",
        "Doorstappen gebeurt alleen wanneer de return of eerste bal ook echt ruimte wint."
      ],
      "right": [
        "Voor rechtshandig links verschuift serve + ball 3 plan right side meer naar backhandcontrole, cross-openingen en later instappen op de volgende bal.",
        "Laat de speler de return iets meer koppelen aan lijnsluiting en partnerlogica.",
        "Werk met dezelfde oefenopbouw, maar andere besluitmomenten rond midden en lijn."
      ]
    },
    "goal": [
      {
        "title": "Hoofddoel",
        "text": "Service gebruiken als start van patroon: serveren, bal drie sturen, netpositie vastleggen."
      },
      {
        "type": "list",
        "title": "Tactische doelen",
        "items": [
          "Situatie herkennen: trigger de eerste bal dwingt je uit comfort. Actie: lees vroeg en kies een functionele eerste oplossing. Waarom: zonder heldere eerste keuze ontstaat chaos.",
          "Patroon afmaken: trigger je eerste bal heeft tijd of ruimte gekocht. Actie: gebruik de tweede bal om richting te geven aan de rally. Waarom: hier verschuift initiatief of controle.",
          "Positie vastleggen: trigger na de laatste bal van de cyclus. Actie: herstel naar de plek waar de volgende bal logisch wordt. Waarom: goede training eindigt niet bij contact, maar bij bruikbare positie."
        ]
      }
    ],
    "setup": [
      {
        "title": "Positie",
        "text": "Start in servicepositie rechts."
      },
      {
        "title": "Setup",
        "text": "Racket klaar voor directe split na service."
      },
      {
        "title": "Setup",
        "text": "Voor linkshandig rechts: service naar backhandvak gevolgd door forehand-/middenvolley is logisch patroon."
      }
    ],
    "targets": [
      {
        "name": "SERVE BACKHAND",
        "text": "Service naar backhand of lastige returnzone."
      },
      {
        "name": "BALL 3 BODY/T",
        "text": "Eerste volley veilig en sturend."
      },
      {
        "name": "NET POSITION",
        "text": "Na bal 3 compact doorbewegen."
      }
    ],
    "steps": [
      {
        "title": "Bal 1",
        "text": "Serveer met richting en herstap direct. Waarom: Patroon openen. Doelzone: SERVE BACKHAND"
      },
      {
        "title": "Bal 2",
        "text": "Lees vroeg en maak split-step. Waarom: Tijd winnen voor bal 3. Doelzone: TRANSITIE"
      },
      {
        "title": "Bal 3",
        "text": "Speel body of T, niet te snel lijn. Waarom: Net dominant innemen. Doelzone: BALL 3 BODY/T"
      }
    ],
    "score": [
      "Bal 1 correct uitgevoerd: __/20",
      "Bal 2 correct uitgevoerd: __/20",
      "Bal 3 correct uitgevoerd: __/20",
      "Fouten totaal: __",
      "Extra KPI: juiste positie na laatste bal: __/20"
    ],
    "coaching": [
      "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
      "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
      "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
    ],
    "machine": [
      "Bal 1: Service. → Serveer met richting en herstap direct. (Patroon openen.)",
      "Bal 2: Returnbal, medium. → Lees vroeg en maak split-step. (Tijd winnen voor bal 3.)",
      "Bal 3: Eerste volley, medium. → Speel body of T, niet te snel lijn. (Net dominant innemen.)"
    ],
    "coachView": [
      {
        "title": "Wat gebeurt er nu vaak?",
        "bullets": [
          "De service wordt te vaak los gezien van de rally die erna komt.",
          "De speler ziet de eerste moeilijke bal wel, maar denkt nog te weinig in een volledig patroon.",
          "Het herstel of de vervolgkeuze komt te laat."
        ]
      },
      {
        "title": "Waarom werkt dit niet?",
        "bullets": [
          "De slag en de positie erna worden nog los van elkaar gezien.",
          "Er is te weinig onderscheid tussen neutraliseren, opbouwen en drukken.",
          "Zonder duidelijk patroon blijft de speler reactief."
        ]
      },
      {
        "title": "Wat je gaat leren",
        "bullets": [
          "Service gebruiken als start van patroon: serveren, bal drie sturen, netpositie vastleggen.",
          "Niet alleen de bal raken, maar ook de volgende positie voorbereiden.",
          "Een herhaalbaar patroon bouwen dat ook onder druk bruikbaar blijft."
        ]
      },
      {
        "title": "Beslisregel",
        "bullets": [
          "🟢 Groen: druk mag omhoog; kies voeten of duidelijke open ruimte als je vóór de bal bent.",
          "🟠 Oranje: kies body of T; controle blijft belangrijker dan spektakel.",
          "🔴 Rood: koop tijd, speel hoog of neutraal en herstel eerst."
        ]
      },
      {
        "title": "Coach Tips",
        "bullets": [
          "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
          "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
          "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
        ]
      }
    ]
  },
  {
    "id": "19",
    "category": "NETDOMINANTIE",
    "title": "Backhand Volley Lock → Middle Squeeze",
    "shortTitle": "Backhand Volley Lock",
    "summary": "Body en midden eerst dichtzetten, dan pas verder openen.",
    "context": "Je staat rechts aan het net en wilt via je backhandvolley het midden kleiner maken.",
    "matchContext": "Spelers zoeken vanaf backhandvolley te snel de lijn en openen juist ruimte.",
    "level": "average",
    "levelLabel": "Gemiddeld",
    "knltb": "6-7",
    "type": "net",
    "typeLabel": "Netdominantie",
    "sequence": "3-ball sequence",
    "repeats": "20 herhalingen",
    "totalBalls": "60 ballen totaal",
    "categoryColor": "#0D3B66",
    "playerVariant": {
      "left": [
        "Voor linkshandig rechts ligt de nadruk op forehand-overheads, bodydruk en het afsluiten van middenballen vanuit backhand volley lock.",
        "Het herstel na overhead of eerste volley blijft iets meer gericht op het bewaken van het midden en de forehandzijde.",
        "Laat de speler na drukballen sneller herkennen wanneer de volgende bal naar voeten/body gaat in plaats van naar de kleine hoek."
      ],
      "right": [
        "Voor rechtshandig links verschuift backhand volley lock meer naar backhand-ordening, lijnbewaking en compacter voetenwerk in de volleyzone.",
        "De nadruk ligt iets vaker op het sluiten van de lijn en daarna pas op het meenemen van middenruimte.",
        "Werk met dezelfde sequentie, maar laat de speler andere contactpunten en herstelhoeken voelen."
      ]
    },
    "goal": [
      {
        "title": "Hoofddoel",
        "text": "Body en midden eerst dichtzetten, dan pas verder openen."
      },
      {
        "type": "list",
        "title": "Tactische doelen",
        "items": [
          "Situatie herkennen: trigger de eerste bal dwingt je uit comfort. Actie: lees vroeg en kies een functionele eerste oplossing. Waarom: zonder heldere eerste keuze ontstaat chaos.",
          "Patroon afmaken: trigger je eerste bal heeft tijd of ruimte gekocht. Actie: gebruik de tweede bal om richting te geven aan de rally. Waarom: hier verschuift initiatief of controle.",
          "Positie vastleggen: trigger na de laatste bal van de cyclus. Actie: herstel naar de plek waar de volgende bal logisch wordt. Waarom: goede training eindigt niet bij contact, maar bij bruikbare positie."
        ]
      }
    ],
    "setup": [
      {
        "title": "Positie",
        "text": "Start aan het net op backhandzijde."
      },
      {
        "title": "Setup",
        "text": "Racketkop hoog, ellebogen rustig."
      },
      {
        "title": "Setup",
        "text": "Voor linkshandig rechts: dit is vaak de stabiele midden-sluiting aan de backhandkant."
      }
    ],
    "targets": [
      {
        "name": "MIDDLE LOCK",
        "text": "Eerste volley naar body of midden."
      },
      {
        "name": "SECOND SQUEEZE",
        "text": "Tweede volley nog compacter en lager."
      },
      {
        "name": "PRESSURE HOLD",
        "text": "Tegenstander in kleine ruimte houden."
      }
    ],
    "steps": [
      {
        "title": "Bal 1",
        "text": "Blok laag naar midden/body. Waarom: Midden sluiten. Doelzone: MIDDLE LOCK"
      },
      {
        "title": "Bal 2",
        "text": "Herhaal druk zonder te openen. Waarom: Ruimte verder verkleinen. Doelzone: SECOND SQUEEZE"
      },
      {
        "title": "Bal 3",
        "text": "Kies voeten of body, alleen hoek bij duidelijk groen. Waarom: Druk vasthouden. Doelzone: PRESSURE HOLD"
      }
    ],
    "score": [
      "Bal 1 correct uitgevoerd: __/20",
      "Bal 2 correct uitgevoerd: __/20",
      "Bal 3 correct uitgevoerd: __/20",
      "Fouten totaal: __",
      "Extra KPI: juiste positie na laatste bal: __/20"
    ],
    "coaching": [
      "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
      "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
      "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
    ],
    "machine": [
      "Bal 1: Backhandvolley, medium. → Blok laag naar midden/body. (Midden sluiten.)",
      "Bal 2: Tweede netbal, medium. → Herhaal druk zonder te openen. (Ruimte verder verkleinen.)",
      "Bal 3: Kansbal, medium. → Kies voeten of body, alleen hoek bij duidelijk groen. (Druk vasthouden.)"
    ],
    "coachView": [
      {
        "title": "Wat gebeurt er nu vaak?",
        "bullets": [
          "Spelers zoeken vanaf backhandvolley te snel de lijn en openen juist ruimte.",
          "De speler ziet de eerste moeilijke bal wel, maar denkt nog te weinig in een volledig patroon.",
          "Het herstel of de vervolgkeuze komt te laat."
        ]
      },
      {
        "title": "Waarom werkt dit niet?",
        "bullets": [
          "De slag en de positie erna worden nog los van elkaar gezien.",
          "Er is te weinig onderscheid tussen neutraliseren, opbouwen en drukken.",
          "Zonder duidelijk patroon blijft de speler reactief."
        ]
      },
      {
        "title": "Wat je gaat leren",
        "bullets": [
          "Body en midden eerst dichtzetten, dan pas verder openen.",
          "Niet alleen de bal raken, maar ook de volgende positie voorbereiden.",
          "Een herhaalbaar patroon bouwen dat ook onder druk bruikbaar blijft."
        ]
      },
      {
        "title": "Beslisregel",
        "bullets": [
          "🟢 Groen: druk mag omhoog; kies voeten of duidelijke open ruimte als je vóór de bal bent.",
          "🟠 Oranje: kies body of T; controle blijft belangrijker dan spektakel.",
          "🔴 Rood: koop tijd, speel hoog of neutraal en herstel eerst."
        ]
      },
      {
        "title": "Coach Tips",
        "bullets": [
          "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
          "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
          "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
        ]
      }
    ]
  },
  {
    "id": "20",
    "category": "BESLISSEN IN TRANSITIE",
    "title": "Transition Lob → Airborne Volley Decision",
    "shortTitle": "Transition Lob",
    "summary": "Beslissen op kleur: voeten als het groen is, T/body als het oranje is.",
    "context": "Je wint tijd met een transitie-lob en krijgt daarna een halfhoge bal tijdens je voorwaartse beweging.",
    "matchContext": "Spelers willen die tweede bal te vaak forceren terwijl ze nog niet geland zijn.",
    "level": "advanced",
    "levelLabel": "Advanced",
    "knltb": "1-5",
    "type": "net",
    "typeLabel": "Netdominantie",
    "sequence": "3-ball sequence",
    "repeats": "20 herhalingen",
    "totalBalls": "60 ballen totaal",
    "categoryColor": "#0D5517",
    "playerVariant": {
      "left": [
        "Voor linkshandig rechts ligt de nadruk op forehand-overheads, bodydruk en het afsluiten van middenballen vanuit transition lob.",
        "Het herstel na overhead of eerste volley blijft iets meer gericht op het bewaken van het midden en de forehandzijde.",
        "Laat de speler na drukballen sneller herkennen wanneer de volgende bal naar voeten/body gaat in plaats van naar de kleine hoek."
      ],
      "right": [
        "Voor rechtshandig links verschuift transition lob meer naar backhand-ordening, lijnbewaking en compacter voetenwerk in de volleyzone.",
        "De nadruk ligt iets vaker op het sluiten van de lijn en daarna pas op het meenemen van middenruimte.",
        "Werk met dezelfde sequentie, maar laat de speler andere contactpunten en herstelhoeken voelen."
      ]
    },
    "goal": [
      {
        "title": "Hoofddoel",
        "text": "Beslissen op kleur: voeten als het groen is, T/body als het oranje is."
      },
      {
        "type": "list",
        "title": "Tactische doelen",
        "items": [
          "Situatie herkennen: trigger de eerste bal dwingt je uit comfort. Actie: lees vroeg en kies een functionele eerste oplossing. Waarom: zonder heldere eerste keuze ontstaat chaos.",
          "Patroon afmaken: trigger je eerste bal heeft tijd of ruimte gekocht. Actie: gebruik de tweede bal om richting te geven aan de rally. Waarom: hier verschuift initiatief of controle.",
          "Positie vastleggen: trigger na de laatste bal van de cyclus. Actie: herstel naar de plek waar de volgende bal logisch wordt. Waarom: goede training eindigt niet bij contact, maar bij bruikbare positie."
        ]
      }
    ],
    "setup": [
      {
        "title": "Positie",
        "text": "Start mid-court of net terugwinnend."
      },
      {
        "title": "Setup",
        "text": "Na lob direct versnellen naar voren."
      },
      {
        "title": "Setup",
        "text": "Voor linkshandig rechts: forehand-scan op de tweede bal is cruciaal."
      }
    ],
    "targets": [
      {
        "name": "TRANSITION LOB",
        "text": "Hoog genoeg om tijd te kopen."
      },
      {
        "name": "AIRBORNE DECISION",
        "text": "Keuze maken vóór contact."
      },
      {
        "name": "SMART PRESSURE",
        "text": "Niet harder, maar slimmer drukken."
      }
    ],
    "steps": [
      {
        "title": "Bal 1",
        "text": "Speel hoge transitie-lob. Waarom: Tijd creëren. Doelzone: TRANSITION LOB"
      },
      {
        "title": "Bal 2",
        "text": "Beslis: T/body of voeten. Waarom: Juiste keuze onder beweging. Doelzone: AIRBORNE DECISION"
      },
      {
        "title": "Bal 3",
        "text": "Zet de gekozen lijn voort en land compact. Waarom: Patroon afmaken. Doelzone: SMART PRESSURE"
      }
    ],
    "score": [
      "Bal 1 correct uitgevoerd: __/20",
      "Bal 2 correct uitgevoerd: __/20",
      "Bal 3 correct uitgevoerd: __/20",
      "Fouten totaal: __",
      "Extra KPI: juiste positie na laatste bal: __/20"
    ],
    "coaching": [
      "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
      "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
      "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
    ],
    "machine": [
      "Bal 1: Lobbal, medium-slow. → Speel hoge transitie-lob. (Tijd creëren.)",
      "Bal 2: Halvhoge bal tijdens instappen, medium. → Beslis: T/body of voeten. (Juiste keuze onder beweging.)",
      "Bal 3: Vervolgvolley, medium. → Zet de gekozen lijn voort en land compact. (Patroon afmaken.)"
    ],
    "coachView": [
      {
        "title": "Wat gebeurt er nu vaak?",
        "bullets": [
          "Spelers willen die tweede bal te vaak forceren terwijl ze nog niet geland zijn.",
          "De speler ziet de eerste moeilijke bal wel, maar denkt nog te weinig in een volledig patroon.",
          "Het herstel of de vervolgkeuze komt te laat."
        ]
      },
      {
        "title": "Waarom werkt dit niet?",
        "bullets": [
          "De slag en de positie erna worden nog los van elkaar gezien.",
          "Er is te weinig onderscheid tussen neutraliseren, opbouwen en drukken.",
          "Zonder duidelijk patroon blijft de speler reactief."
        ]
      },
      {
        "title": "Wat je gaat leren",
        "bullets": [
          "Beslissen op kleur: voeten als het groen is, T/body als het oranje is.",
          "Niet alleen de bal raken, maar ook de volgende positie voorbereiden.",
          "Een herhaalbaar patroon bouwen dat ook onder druk bruikbaar blijft."
        ]
      },
      {
        "title": "Beslisregel",
        "bullets": [
          "🟢 Groen: druk mag omhoog; kies voeten of duidelijke open ruimte als je vóór de bal bent.",
          "🟠 Oranje: kies body of T; controle blijft belangrijker dan spektakel.",
          "🔴 Rood: koop tijd, speel hoog of neutraal en herstel eerst."
        ]
      },
      {
        "title": "Coach Tips",
        "bullets": [
          "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
          "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
          "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
        ]
      }
    ]
  },
  {
    "id": "21",
    "category": "EXTREME DEFENSE",
    "title": "Double Glass Escape → Slow Rebuild",
    "shortTitle": "Double Glass Escape",
    "summary": "Voetenwerk vóór swing, hoogte vóór precisie, opbouw vóór aanval.",
    "context": "De bal werkt dubbel via zij- en achterglas en dwingt je tot overleven met voetenwerk en rust.",
    "matchContext": "In dubbele glasdruk willen spelers te snel weer neutraal worden en maken ze techniekfouten.",
    "level": "advanced",
    "levelLabel": "Advanced",
    "knltb": "1-5",
    "type": "defense",
    "typeLabel": "Defense & recovery",
    "sequence": "3-ball sequence",
    "repeats": "20 herhalingen",
    "totalBalls": "60 ballen totaal",
    "categoryColor": "#6B4226",
    "playerVariant": {
      "left": [
        "Voor linkshandig rechts krijgt double glass escape meer nadruk op forehand-reddingen, uit de hoek draaien en via cross of midden tijd kopen.",
        "De herstelpas richt zich vaker op het opnieuw vrijmaken van de forehandhelft en het terugclaimen van middenruimte.",
        "Laat de speler herkennen wanneer een hoge reset slimmer is dan een vlakke uitweg."
      ],
      "right": [
        "Voor rechtshandig links ligt double glass escape meer op backhand-organisatie, compacter glasgebruik en rust houden richting lijnzijde.",
        "De speler moet sneller kiezen tussen blokkeren, hoog resetten of via midden neutraliseren.",
        "Herstel blijft leidend: eerst uit de druk, daarna pas denken aan omschakelen."
      ]
    },
    "goal": [
      {
        "title": "Hoofddoel",
        "text": "Voetenwerk vóór swing, hoogte vóór precisie, opbouw vóór aanval."
      },
      {
        "type": "list",
        "title": "Tactische doelen",
        "items": [
          "Situatie herkennen: trigger de eerste bal dwingt je uit comfort. Actie: lees vroeg en kies een functionele eerste oplossing. Waarom: zonder heldere eerste keuze ontstaat chaos.",
          "Patroon afmaken: trigger je eerste bal heeft tijd of ruimte gekocht. Actie: gebruik de tweede bal om richting te geven aan de rally. Waarom: hier verschuift initiatief of controle.",
          "Positie vastleggen: trigger na de laatste bal van de cyclus. Actie: herstel naar de plek waar de volgende bal logisch wordt. Waarom: goede training eindigt niet bij contact, maar bij bruikbare positie."
        ]
      }
    ],
    "setup": [
      {
        "title": "Positie",
        "text": "Start diep in de hoek, verwacht complexe stuit."
      },
      {
        "title": "Setup",
        "text": "Lichaam laag, kleine aanpassingspassen."
      },
      {
        "title": "Setup",
        "text": "Voor linkshandig rechts: backhand- en forehandhoek spiegelen in aparte sets."
      }
    ],
    "targets": [
      {
        "name": "ESCAPE BALL",
        "text": "Bal één veilig omhoog."
      },
      {
        "name": "SLOW REBUILD",
        "text": "Tweede bal gecontroleerd naar T."
      },
      {
        "name": "STABLE BASE",
        "text": "Na elke slag terug naar werkbare basis."
      }
    ],
    "steps": [
      {
        "title": "Bal 1",
        "text": "Overleef met hoge veilige lift of lob. Waarom: Rally in leven houden. Doelzone: ESCAPE BALL"
      },
      {
        "title": "Bal 2",
        "text": "Bouw rustig op naar T. Waarom: Van chaos naar rust. Doelzone: SLOW REBUILD"
      },
      {
        "title": "Bal 3",
        "text": "Blijf compact en stabiel. Waarom: Geen haast naar aanval. Doelzone: STABLE BASE"
      }
    ],
    "score": [
      "Bal 1 correct uitgevoerd: __/20",
      "Bal 2 correct uitgevoerd: __/20",
      "Bal 3 correct uitgevoerd: __/20",
      "Fouten totaal: __",
      "Extra KPI: juiste positie na laatste bal: __/20"
    ],
    "coaching": [
      "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
      "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
      "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
    ],
    "machine": [
      "Bal 1: Dubbele glasbal, lastig. → Overleef met hoge veilige lift of lob. (Rally in leven houden.)",
      "Bal 2: Bruikbare volgende bal, medium. → Bouw rustig op naar T. (Van chaos naar rust.)",
      "Bal 3: Neutrale rallybal, medium. → Blijf compact en stabiel. (Geen haast naar aanval.)"
    ],
    "coachView": [
      {
        "title": "Wat gebeurt er nu vaak?",
        "bullets": [
          "In dubbele glasdruk willen spelers te snel weer neutraal worden en maken ze techniekfouten.",
          "De speler ziet de eerste moeilijke bal wel, maar denkt nog te weinig in een volledig patroon.",
          "Het herstel of de vervolgkeuze komt te laat."
        ]
      },
      {
        "title": "Waarom werkt dit niet?",
        "bullets": [
          "De slag en de positie erna worden nog los van elkaar gezien.",
          "Er is te weinig onderscheid tussen neutraliseren, opbouwen en drukken.",
          "Zonder duidelijk patroon blijft de speler reactief."
        ]
      },
      {
        "title": "Wat je gaat leren",
        "bullets": [
          "Voetenwerk vóór swing, hoogte vóór precisie, opbouw vóór aanval.",
          "Niet alleen de bal raken, maar ook de volgende positie voorbereiden.",
          "Een herhaalbaar patroon bouwen dat ook onder druk bruikbaar blijft."
        ]
      },
      {
        "title": "Beslisregel",
        "bullets": [
          "🟢 Groen: druk mag omhoog; kies voeten of duidelijke open ruimte als je vóór de bal bent.",
          "🟠 Oranje: kies body of T; controle blijft belangrijker dan spektakel.",
          "🔴 Rood: koop tijd, speel hoog of neutraal en herstel eerst."
        ]
      },
      {
        "title": "Coach Tips",
        "bullets": [
          "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
          "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
          "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
        ]
      }
    ]
  },
  {
    "id": "22",
    "category": "OVERHEAD DRUK",
    "title": "Counterbandeja → Step In → Finish Window",
    "shortTitle": "Counterbandeja",
    "summary": "Eerst druk terugleggen, daarna direct naar voren en de volgende bal laag afmaken of vastzetten.",
    "context": "Na een gecontroleerde tegenbandeja herken je het moment waarop je zelf weer mag instappen.",
    "matchContext": "Spelers zien de counterbandeja als eindslag, terwijl het vaak de openingsslag is.",
    "level": "advanced",
    "levelLabel": "Advanced",
    "knltb": "1-5",
    "type": "net",
    "typeLabel": "Netdominantie",
    "sequence": "3-ball sequence",
    "repeats": "20 herhalingen",
    "totalBalls": "60 ballen totaal",
    "categoryColor": "#0D3B66",
    "playerVariant": {
      "left": [
        "Voor linkshandig rechts ligt de nadruk op forehand-overheads, bodydruk en het afsluiten van middenballen vanuit counterbandeja.",
        "Het herstel na overhead of eerste volley blijft iets meer gericht op het bewaken van het midden en de forehandzijde.",
        "Laat de speler na drukballen sneller herkennen wanneer de volgende bal naar voeten/body gaat in plaats van naar de kleine hoek."
      ],
      "right": [
        "Voor rechtshandig links verschuift counterbandeja meer naar backhand-ordening, lijnbewaking en compacter voetenwerk in de volleyzone.",
        "De nadruk ligt iets vaker op het sluiten van de lijn en daarna pas op het meenemen van middenruimte.",
        "Werk met dezelfde sequentie, maar laat de speler andere contactpunten en herstelhoeken voelen."
      ]
    },
    "goal": [
      {
        "title": "Hoofddoel",
        "text": "Eerst druk terugleggen, daarna direct naar voren en de volgende bal laag afmaken of vastzetten."
      },
      {
        "type": "list",
        "title": "Tactische doelen",
        "items": [
          "Situatie herkennen: trigger de eerste bal dwingt je uit comfort. Actie: lees vroeg en kies een functionele eerste oplossing. Waarom: zonder heldere eerste keuze ontstaat chaos.",
          "Patroon afmaken: trigger je eerste bal heeft tijd of ruimte gekocht. Actie: gebruik de tweede bal om richting te geven aan de rally. Waarom: hier verschuift initiatief of controle.",
          "Positie vastleggen: trigger na de laatste bal van de cyclus. Actie: herstel naar de plek waar de volgende bal logisch wordt. Waarom: goede training eindigt niet bij contact, maar bij bruikbare positie."
        ]
      }
    ],
    "setup": [
      {
        "title": "Positie",
        "text": "Start net of iets daarachter, klaar voor overhead."
      },
      {
        "title": "Setup",
        "text": "Werk met rustige overheadbasis en snelle herstap."
      },
      {
        "title": "Setup",
        "text": "Voor linkshandig rechts: forehand-overhead naar midden/body gevolgd door instapbal."
      }
    ],
    "targets": [
      {
        "name": "COUNTERBANDEJA",
        "text": "Diep en bruikbaar, niet hard."
      },
      {
        "name": "STEP IN",
        "text": "Na contact meteen naar voren."
      },
      {
        "name": "FINISH WINDOW",
        "text": "Volgende bal laag naar voeten of open hoek."
      }
    ],
    "steps": [
      {
        "title": "Bal 1",
        "text": "Speel gecontroleerde counterbandeja. Waarom: Tegenstander opnieuw onder druk zetten. Doelzone: COUNTERBANDEJA"
      },
      {
        "title": "Bal 2",
        "text": "Stap in en neem vroeg. Waarom: Venster openen. Doelzone: STEP IN"
      },
      {
        "title": "Bal 3",
        "text": "Kies lage finish of stevige controlevolley. Waarom: Druk verzilveren. Doelzone: FINISH WINDOW"
      }
    ],
    "score": [
      "Bal 1 correct uitgevoerd: __/20",
      "Bal 2 correct uitgevoerd: __/20",
      "Bal 3 correct uitgevoerd: __/20",
      "Fouten totaal: __",
      "Extra KPI: juiste positie na laatste bal: __/20"
    ],
    "coaching": [
      "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
      "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
      "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
    ],
    "machine": [
      "Bal 1: Lob/overheadbal, medium-slow. → Speel gecontroleerde counterbandeja. (Tegenstander opnieuw onder druk zetten.)",
      "Bal 2: Korte of halfkorte reply, medium. → Stap in en neem vroeg. (Venster openen.)",
      "Bal 3: Kansbal, medium. → Kies lage finish of stevige controlevolley. (Druk verzilveren.)"
    ],
    "coachView": [
      {
        "title": "Wat gebeurt er nu vaak?",
        "bullets": [
          "Spelers zien de counterbandeja als eindslag, terwijl het vaak de openingsslag is.",
          "De speler ziet de eerste moeilijke bal wel, maar denkt nog te weinig in een volledig patroon.",
          "Het herstel of de vervolgkeuze komt te laat."
        ]
      },
      {
        "title": "Waarom werkt dit niet?",
        "bullets": [
          "De slag en de positie erna worden nog los van elkaar gezien.",
          "Er is te weinig onderscheid tussen neutraliseren, opbouwen en drukken.",
          "Zonder duidelijk patroon blijft de speler reactief."
        ]
      },
      {
        "title": "Wat je gaat leren",
        "bullets": [
          "Eerst druk terugleggen, daarna direct naar voren en de volgende bal laag afmaken of vastzetten.",
          "Niet alleen de bal raken, maar ook de volgende positie voorbereiden.",
          "Een herhaalbaar patroon bouwen dat ook onder druk bruikbaar blijft."
        ]
      },
      {
        "title": "Beslisregel",
        "bullets": [
          "🟢 Groen: druk mag omhoog; kies voeten of duidelijke open ruimte als je vóór de bal bent.",
          "🟠 Oranje: kies body of T; controle blijft belangrijker dan spektakel.",
          "🔴 Rood: koop tijd, speel hoog of neutraal en herstel eerst."
        ]
      },
      {
        "title": "Coach Tips",
        "bullets": [
          "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
          "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
          "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
        ]
      }
    ]
  },
  {
    "id": "23",
    "category": "RUIMTE CREËREN",
    "title": "Body Defense → Open Space Cross",
    "shortTitle": "Body Defense",
    "summary": "Eerst blokkeren naar midden/body, daarna openen als de ruimte echt ontstaan is.",
    "context": "Je krijgt een lastige bodybal en moet eerst klein maken voordat je de vrijgekomen crossruimte benut.",
    "matchContext": "Spelers willen meteen hoek spelen uit een benarde body-situatie.",
    "level": "beginner",
    "levelLabel": "Beginner",
    "knltb": "8-9",
    "type": "defense",
    "typeLabel": "Defense & recovery",
    "sequence": "3-ball sequence",
    "repeats": "20 herhalingen",
    "totalBalls": "60 ballen totaal",
    "categoryColor": "#4A1C6A",
    "playerVariant": {
      "left": [
        "Voor linkshandig rechts krijgt body defense meer nadruk op forehand-reddingen, uit de hoek draaien en via cross of midden tijd kopen.",
        "De herstelpas richt zich vaker op het opnieuw vrijmaken van de forehandhelft en het terugclaimen van middenruimte.",
        "Laat de speler herkennen wanneer een hoge reset slimmer is dan een vlakke uitweg."
      ],
      "right": [
        "Voor rechtshandig links ligt body defense meer op backhand-organisatie, compacter glasgebruik en rust houden richting lijnzijde.",
        "De speler moet sneller kiezen tussen blokkeren, hoog resetten of via midden neutraliseren.",
        "Herstel blijft leidend: eerst uit de druk, daarna pas denken aan omschakelen."
      ]
    },
    "goal": [
      {
        "title": "Hoofddoel",
        "text": "Eerst blokkeren naar midden/body, daarna openen als de ruimte echt ontstaan is."
      },
      {
        "type": "list",
        "title": "Tactische doelen",
        "items": [
          "Situatie herkennen: trigger de eerste bal dwingt je uit comfort. Actie: lees vroeg en kies een functionele eerste oplossing. Waarom: zonder heldere eerste keuze ontstaat chaos.",
          "Patroon afmaken: trigger je eerste bal heeft tijd of ruimte gekocht. Actie: gebruik de tweede bal om richting te geven aan de rally. Waarom: hier verschuift initiatief of controle.",
          "Positie vastleggen: trigger na de laatste bal van de cyclus. Actie: herstel naar de plek waar de volgende bal logisch wordt. Waarom: goede training eindigt niet bij contact, maar bij bruikbare positie."
        ]
      }
    ],
    "setup": [
      {
        "title": "Positie",
        "text": "Start in neutrale verdedigingspositie."
      },
      {
        "title": "Setup",
        "text": "Racket compact dicht bij lichaam."
      },
      {
        "title": "Setup",
        "text": "Voor linkshandig rechts: gebruik body-block als stabiele tussenslag."
      }
    ],
    "targets": [
      {
        "name": "BODY BLOCK",
        "text": "Eerste bal compact in spel."
      },
      {
        "name": "CROSS WINDOW",
        "text": "Tweede bal naar vrijgekomen crossruimte."
      },
      {
        "name": "RECOVER",
        "text": "Na openen weer in balans."
      }
    ],
    "steps": [
      {
        "title": "Bal 1",
        "text": "Blok kort en compact naar midden/body. Waarom: Eerst neutraliseren. Doelzone: BODY BLOCK"
      },
      {
        "title": "Bal 2",
        "text": "Open bewust cross. Waarom: Pas ruimte gebruiken als hij er is. Doelzone: CROSS WINDOW"
      },
      {
        "title": "Bal 3",
        "text": "Herstel en blijf in balans. Waarom: Ruimte benutten zonder overforceren. Doelzone: RECOVER"
      }
    ],
    "score": [
      "Bal 1 correct uitgevoerd: __/20",
      "Bal 2 correct uitgevoerd: __/20",
      "Bal 3 correct uitgevoerd: __/20",
      "Fouten totaal: __",
      "Extra KPI: juiste positie na laatste bal: __/20"
    ],
    "coaching": [
      "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
      "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
      "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
    ],
    "machine": [
      "Bal 1: Bodybal, medium-fast. → Blok kort en compact naar midden/body. (Eerst neutraliseren.)",
      "Bal 2: Bruikbare vervolgbal, medium. → Open bewust cross. (Pas ruimte gebruiken als hij er is.)",
      "Bal 3: Neutrale bal, medium. → Herstel en blijf in balans. (Ruimte benutten zonder overforceren.)"
    ],
    "coachView": [
      {
        "title": "Wat gebeurt er nu vaak?",
        "bullets": [
          "Spelers willen meteen hoek spelen uit een benarde body-situatie.",
          "De speler ziet de eerste moeilijke bal wel, maar denkt nog te weinig in een volledig patroon.",
          "Het herstel of de vervolgkeuze komt te laat."
        ]
      },
      {
        "title": "Waarom werkt dit niet?",
        "bullets": [
          "De slag en de positie erna worden nog los van elkaar gezien.",
          "Er is te weinig onderscheid tussen neutraliseren, opbouwen en drukken.",
          "Zonder duidelijk patroon blijft de speler reactief."
        ]
      },
      {
        "title": "Wat je gaat leren",
        "bullets": [
          "Eerst blokkeren naar midden/body, daarna openen als de ruimte echt ontstaan is.",
          "Niet alleen de bal raken, maar ook de volgende positie voorbereiden.",
          "Een herhaalbaar patroon bouwen dat ook onder druk bruikbaar blijft."
        ]
      },
      {
        "title": "Beslisregel",
        "bullets": [
          "🟢 Groen: druk mag omhoog; kies voeten of duidelijke open ruimte als je vóór de bal bent.",
          "🟠 Oranje: kies body of T; controle blijft belangrijker dan spektakel.",
          "🔴 Rood: koop tijd, speel hoog of neutraal en herstel eerst."
        ]
      },
      {
        "title": "Coach Tips",
        "bullets": [
          "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
          "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
          "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
        ]
      }
    ]
  },
  {
    "id": "24",
    "category": "SEQUENCING AAN HET NET",
    "title": "Freeze Volley → Angle Open → Finish",
    "shortTitle": "Freeze Volley",
    "summary": "Volgorde trainen: freeze, openen, afmaken of vasthouden.",
    "context": "Je hebt druk aan het net, maar nog geen echte winnaar. Eerst vastzetten, dan pas openen.",
    "matchContext": "Veel spelers slaan te vroeg de hoek en geven de rally weer weg.",
    "level": "advanced",
    "levelLabel": "Advanced",
    "knltb": "1-5",
    "type": "net",
    "typeLabel": "Netdominantie",
    "sequence": "3-ball sequence",
    "repeats": "20 herhalingen",
    "totalBalls": "60 ballen totaal",
    "categoryColor": "#0D3B66",
    "playerVariant": {
      "left": [
        "Voor linkshandig rechts ligt de nadruk op forehand-overheads, bodydruk en het afsluiten van middenballen vanuit freeze volley.",
        "Het herstel na overhead of eerste volley blijft iets meer gericht op het bewaken van het midden en de forehandzijde.",
        "Laat de speler na drukballen sneller herkennen wanneer de volgende bal naar voeten/body gaat in plaats van naar de kleine hoek."
      ],
      "right": [
        "Voor rechtshandig links verschuift freeze volley meer naar backhand-ordening, lijnbewaking en compacter voetenwerk in de volleyzone.",
        "De nadruk ligt iets vaker op het sluiten van de lijn en daarna pas op het meenemen van middenruimte.",
        "Werk met dezelfde sequentie, maar laat de speler andere contactpunten en herstelhoeken voelen."
      ]
    },
    "goal": [
      {
        "title": "Hoofddoel",
        "text": "Volgorde trainen: freeze, openen, afmaken of vasthouden."
      },
      {
        "type": "list",
        "title": "Tactische doelen",
        "items": [
          "Situatie herkennen: trigger de eerste bal dwingt je uit comfort. Actie: lees vroeg en kies een functionele eerste oplossing. Waarom: zonder heldere eerste keuze ontstaat chaos.",
          "Patroon afmaken: trigger je eerste bal heeft tijd of ruimte gekocht. Actie: gebruik de tweede bal om richting te geven aan de rally. Waarom: hier verschuift initiatief of controle.",
          "Positie vastleggen: trigger na de laatste bal van de cyclus. Actie: herstel naar de plek waar de volgende bal logisch wordt. Waarom: goede training eindigt niet bij contact, maar bij bruikbare positie."
        ]
      }
    ],
    "setup": [
      {
        "title": "Positie",
        "text": "Start in net-homebase, licht voorover."
      },
      {
        "title": "Setup",
        "text": "Racket compact, korte swing."
      },
      {
        "title": "Setup",
        "text": "Voor linkshandig rechts: eerste volley vaak forehand naar body, tweede bal opent pas."
      }
    ],
    "targets": [
      {
        "name": "FREEZE",
        "text": "Eerste volley naar body of midden."
      },
      {
        "name": "OPEN ANGLE",
        "text": "Tweede bal pas naar hoek of voeten."
      },
      {
        "name": "FINISH/HOLD",
        "text": "Derde bal afmaken of positie behouden."
      }
    ],
    "steps": [
      {
        "title": "Bal 1",
        "text": "Vastzetten met body/middenvolley. Waarom: Tegenstander klein houden. Doelzone: FREEZE"
      },
      {
        "title": "Bal 2",
        "text": "Open pas nu de hoek of speel naar voeten. Waarom: Ruimte gebruiken op het juiste moment. Doelzone: OPEN ANGLE"
      },
      {
        "title": "Bal 3",
        "text": "Kies finish of veilige lock-volley. Waarom: Punt structureren. Doelzone: FINISH/HOLD"
      }
    ],
    "score": [
      "Bal 1 correct uitgevoerd: __/20",
      "Bal 2 correct uitgevoerd: __/20",
      "Bal 3 correct uitgevoerd: __/20",
      "Fouten totaal: __",
      "Extra KPI: juiste positie na laatste bal: __/20"
    ],
    "coaching": [
      "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
      "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
      "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
    ],
    "machine": [
      "Bal 1: Netbal, medium. → Vastzetten met body/middenvolley. (Tegenstander klein houden.)",
      "Bal 2: Tweede netbal, medium. → Open pas nu de hoek of speel naar voeten. (Ruimte gebruiken op het juiste moment.)",
      "Bal 3: Kansbal, medium. → Kies finish of veilige lock-volley. (Punt structureren.)"
    ],
    "coachView": [
      {
        "title": "Wat gebeurt er nu vaak?",
        "bullets": [
          "Veel spelers slaan te vroeg de hoek en geven de rally weer weg.",
          "De speler ziet de eerste moeilijke bal wel, maar denkt nog te weinig in een volledig patroon.",
          "Het herstel of de vervolgkeuze komt te laat."
        ]
      },
      {
        "title": "Waarom werkt dit niet?",
        "bullets": [
          "De slag en de positie erna worden nog los van elkaar gezien.",
          "Er is te weinig onderscheid tussen neutraliseren, opbouwen en drukken.",
          "Zonder duidelijk patroon blijft de speler reactief."
        ]
      },
      {
        "title": "Wat je gaat leren",
        "bullets": [
          "Volgorde trainen: freeze, openen, afmaken of vasthouden.",
          "Niet alleen de bal raken, maar ook de volgende positie voorbereiden.",
          "Een herhaalbaar patroon bouwen dat ook onder druk bruikbaar blijft."
        ]
      },
      {
        "title": "Beslisregel",
        "bullets": [
          "🟢 Groen: druk mag omhoog; kies voeten of duidelijke open ruimte als je vóór de bal bent.",
          "🟠 Oranje: kies body of T; controle blijft belangrijker dan spektakel.",
          "🔴 Rood: koop tijd, speel hoog of neutraal en herstel eerst."
        ]
      },
      {
        "title": "Coach Tips",
        "bullets": [
          "Denk in waarom → wat → hoe: eerst het doel van de bal, dan pas de uitvoering.",
          "De kwaliteit van de herstelpas na contact is minstens zo belangrijk als de slag zelf.",
          "Laat de speler hardop benoemen wanneer de rally van defense naar controle of van controle naar druk gaat."
        ]
      }
    ]
  }
];
